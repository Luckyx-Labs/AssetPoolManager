...
contract AssetPoolManagerTest is Test {
    AssetPoolManagerMultiSig public assetPoolManagerMultiSig;
    ...
    function setUp() public {
        ...
        assetPoolManagerMultiSig = AssetPoolManagerMultiSig(payable(address(proxy)));
    ...
    function testETHDeposit() public {
        ...
        assertEq(assetPoolManagerMultiSig.poolBalance(assetPoolManagerMultiSig.ETH_ADDRESS()), 0);
        
        // deposit ETH
        vm.prank(user1);
        assetPoolManagerMultiSig.depositETH{value: depositAmount}();
        
        // check after deposit - no fee charged on deposit
        assertEq(assetPoolManagerMultiSig.poolBalance(assetPoolManagerMultiSig.ETH_ADDRESS()), depositAmount);
    }
    ...
    function testETHWithdraw() public {
        ...
        // withdraw ETH
        vm.prank(user1);
        assetPoolManagerMultiSig.withdrawETH(withdrawAmount, user2);
    ...
    function test_RevertWhen_DirectETHTransfer() public {
        ...
        (bool success, ) = address(assetPoolManagerMultiSig).call{value: 1 ether}("");
        success; // avoid unused variable warnings
    }
    
    function testAdminRole() public view {
        // verify admin has ADMIN_ROLE
        assertTrue(assetPoolManagerMultiSig.hasRole(assetPoolManagerMultiSig.ADMIN_ROLE(), admin));
        assertTrue(assetPoolManagerMultiSig.hasRole(assetPoolManagerMultiSig.DEFAULT_ADMIN_ROLE(), admin));
    }

    // test get pool all balances
    function testGetPoolAllBalances() public {
        vm.startPrank(admin);
        (address[] memory tokens, uint256[] memory balances) = assetPoolManagerMultiSig.getPoolAllBalances();
        vm.stopPrank();
        assertEq(tokens.length, balances.length);
        for (uint256 i = 0; i < tokens.length; i++) {
            assertEq(assetPoolManagerMultiSig.poolBalance(tokens[i]), balances[i]);
        }
    }
